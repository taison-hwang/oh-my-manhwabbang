...
## E-52 — **읽을 수 있는 것은 읽는다: 중앙 디렉터리가 없어도 로컬 헤더가 있다** (28세션차, 2026-08-27)

**요청.** 사용자: *"손상된 ZIP 9건 전수 분류해줘"*, 이어서 *"1,2,3 다 처리해 주세요"*.

### 1. 라벨 하나가 원인 넷을 덮고 있었다

인덱스의 `error 9`는 전부 같은 문자열을 달고 있었다 — `zip: end of central directory not found`.
그건 **증상**이다. 바이트를 읽으면 원인은 넷이고, 처방이 서로 다르다.

| 형태 | 건수 | 특징 |
|---|---|---|
| 꼬리 0-채움 | 3 | 파일 길이는 온전, 꼬리가 NUL. 0 시작점은 블록 경계에 안 맞는다 |
| 잘림 | 3 | 길이 자체가 짧다. 군계 두 권은 **정확히 4KiB 배수**에서 끊겼다 |
| 중간 손상 + 꼬리 손실 | 2 | 파일 한가운데 비-0 쓰레기, **그 뒤로 멀쩡한 엔트리가 더 있다** |
| 0바이트 | 1 | 회수 불가 |

**740쪽 중 733쪽(99.1%)이 CRC 검증을 통과하는 상태로 파일 안에 있었다.**

**"소스 하나 불량" 가설은 기각.** 9건은 8개 시리즈에 흩어져 있고 mtime은 2012~2016년이며 같은
배치의 형제 볼륨은 전부 정상이다. [[measure-before-believing-the-diagnosis]]의 재판이다.

### 2. 판정 — `nil`을 돌려주는 것은 없는 색인을 위해 읽히는 책을 버리는 것이다

ZIP은 뒤에서 앞으로 읽는다. end record가 중앙 디렉터리를, 디렉터리가 각 엔트리를 가리킨다. **둘 다
파일 끝에** 있다. 그래서 다운로드가 끊기거나 꼬리가 0으로 덮이는 두 사고는 **지도만 지우고 영토는
남긴다.** 엔트리마다 자기 앞에 30바이트 로컬 헤더가 있고 그건 파일 전체에 흩어져 있다.

**구조적 실패에서 로컬 헤더를 걷는다**(`internal/archive/zipidx/salvage.go`). 찾은 엔트리는
**에러와 함께** 돌려준다 — 책은 읽히되 손상 배지와 scan_log 행을 유지한다. FR-IDX-010이 요구하는
것은 격리이지 삭제가 아니다. 차등 오라클이 보는 **판정은 불변**이므로 archive/zip과 9건 전부에서
여전히 일치한다. FR-IDX-002도 지킨다 — 로컬 헤더가 압축 크기를 들고 있으므로 페이로드는 **읽지 않고
건너뛴다.**

### 3. 워크는 체인이자 검색이어야 한다, 그리고 재동기화 시작점이 정답을 가른다

- 시그니처 스캔만: 압축 데이터 속 `PK\x03\x04`를 엔트리로 오인 → 유령이 쓰레기를 서빙한다.
- 체인만: 첫 손상에서 멈춰 그 뒤를 통째로 버린다 (`유레카26` 3/91, `최종병기그녀 06권` 16/147).

그리고 끊겼을 때 **어디서부터 다시 찾느냐**가 한 엔트리를 더 가른다. 체인이 착지한 지점은 증거가
아니다 — 거기로 보낸 *마지막 헤더의 압축 크기 필드*가 바로 틀린 그 필드이기 때문이다. 그래서 검색은
**마지막으로 믿은 페이로드 안에서** 다시 시작하고, 워터마크가 앞으로만 움직여 전체를 선형으로 묶는다.

### 4. 퍼즈가 잡은 것 둘 — 둘 다 조용했다

1. **버퍼 오버런.** 시그니처 스캔이 `buf[i+3]`을 읽는데 루프 상한이 `len(buf)`였다. 30바이트짜리
   `"0"*29 + "P"` 입력에서 패닉. **퍼즈가 아니었으면 스캐너를 죽였을 것이다.**
2. **짧은 캐시 창.** `window()`가 캐시된 창의 꼬리를 그대로 돌려줘, 호출자가 아카이브가 29바이트
   일찍 끝난 것처럼 보게 만들었다. 패닉이 아니라 **조용한 유실**이라 테스트로 잡기 어려웠다 —
   경계 산술을 종이에 적다 나왔다. 두 경우 모두 회귀 테스트를 붙였다.

수정 후 **2,838만 회 실행, 크래시 0.**

### 5. 파일 복구 — `zip -FF`를 쓰지 않은 이유

`zip -FF`는 로컬 헤더에서 디렉터리를 재구성하되 **엔트리가 온전한지 구분하지 못한다.** 손상 엔트리를
멀쩡한 것과 똑같이 목록에 넣고, 독자가 그 페이지에서 만난다. 그래서 **CRC를 통과한 엔트리만** 담는
재구성기를 썼다. 압축 바이트는 그대로 복사하고 원본 이름 바이트와 플래그를 함께 옮기므로 CP949
이름이 이전과 똑같이 디코드된다(이름 대조 933/933 일치).

검증 3중: python `zipfile.testzip()` · shelf 자신의 리더가 `err=nil` · 이름 전량 일치.
**8건 교체, 원본은 루트 밖 보관함으로 이동, 아무것도 삭제하지 않았다.**

### 6. 그러다 드러난 것 — **`ok`는 페이로드를 본 적이 없다**

`군계(軍鷄) 02권.zip`은 인덱스에 `status=ok`, 201쪽이었다. 실제로는 `군계 02-197.jpg`가 **CRC
불일치**다. FR-IDX-002가 스캔을 중앙 디렉터리 읽기로 한정하므로 **스캐너는 페이로드를 검증하지
않는다** — `ok`는 *"목차가 읽힌다"* 이지 *"페이지가 읽힌다"* 가 아니다.

이 한 건은 마침 압축 푼 디렉터리에 온전한 사본이 있어 수리했다. **모수는 모른다.** §5.7 `bv`로
열어 둔다 — 전량 CRC 스윕은 1.5 TB 읽기이고, 사용자가 요청하지 않았다.

이것도 [[checks-that-watch-the-wrong-thing]]의 한 형태다. 다만 이번엔 검사가 엉뚱한 것을 본 게
아니라, **아예 보지 않기로 한 것**을 `ok`라는 단어가 본 것처럼 읽히게 하고 있었다.

---

## E-53 — **서체 벤더링을 끝낸다: 스택이 시스템 폰트에 닿기 전에 벤더 서체에 닿아야 한다** (28세션차, 2026-08-27)

**요청.** 사용자: *"현재 적용된 폰트가 이 웹페이지에 접속한 모든 디바이스에서 적용되어 보여질 수
있도록"*, 이어서 *"해당 폰트가 설치되어 있지 않았던 디바이스에도"*.

### 1. E-46은 옳았고, 절반만 실행돼 있었다

E-46은 고운바탕을 벤더링하며 **명조가 곧 디자인이고 시스템 산세리프 대체는 다른 제품**이라고
못박았다. 그런데 구멍이 둘 남아 있었다.

- **한자·가나** — 고운바탕엔 한자가 0자다(서브셋 선택이 아니라 원본이 그렇다). 시리즈 제목의 한자는
  `Noto Serif KR` → `Apple SD Gothic Neo` → `serif`로 떨어졌다. 이 리눅스 기계엔 있고 맥엔 있고
  **윈도우와 폰엔 없다** = 두부 상자.
- **`--font-ui`** — `ui-sans-serif, 'Helvetica Neue', Arial, sans-serif`, **네 항목 전부 시스템
  폰트**. 숫자와 대문자 미세 라벨을 그리는 토큰이므로(명조 숫자는 비례폭이라 표에서 안 맞는다),
  쪽수·용량·시각·모든 kicker가 **제품에서 마지막까지 기기마다 다르게 그려지던 텍스트**였다.

### 2. 커버리지는 표본이 아니라 닫힌 집합이다

책·시리즈 이름은 **파일명에서** 오고 `internal/kenc`가 UTF-8 · CP949 · Shift_JIS로 디코드한다.
그러므로 **레거시 인코딩 이름이 담을 수 있는 한자는 그 두 인코딩이 표현 가능한 것 전부**다 —
CP949 4,888자, Shift_JIS 6,356자, 합집합 **7,159자**. 여기에 가나 두 블록(192)을 더해 **7,351자**.

라이브 실측으로 확인: 18,729개 이름에 한자 111자·가나 69자, **전부 그 안에 있다**(당연히, 구성상).
내년에 추가될 책이 커버되는 이유는 우리가 살펴봤기 때문이 아니라 **인코딩이 그 밖을 표현할 수 없기
때문**이다. 남는 구멍은 UTF-8로 저장된 이름이 두 레거시 집합 밖의 한자를 담는 경우뿐이고, 그건
지금까지와 똑같이 폴백한다.

레시피는 `scripts/mkfonts/subset.py`다 — 표가 아니라 **코덱을 왕복시켜** 집합을 만들므로 kenc가
디코드할 것과 커버리지가 어긋날 수 없다.

### 3. 3 MB를 감당 가능하게 만드는 것은 `unicode-range`다

선언이 CJK 블록만 주장하므로 브라우저는 **한자나 가나를 실제로 그리는 화면에서만** 이 파일을
받는다. 한글 제목만 있는 라이브러리는 평생 요청하지 않는다. 한글은 의도적으로 범위 **밖**이다 —
고운바탕의 것이고, 본명조가 답하게 두면 스킨의 명조 한글이 조용히 Noto의 것으로 바뀐다.

`--font-ui`는 반대로 `unicode-range`가 없다. 숫자가 있는 모든 화면에 필요하고, Archivo 라틴 컷은
두 굵기 합쳐 **29,208바이트**다.

| | 크기 |
|---|---|
| 본명조 한자·가나 (7,351자, 두 굵기) | 2,997,496 B |
| Archivo 라틴 (두 굵기) | 29,208 B |
| **합계** | **3,026,704 B** |
| 바이너리 | **30,351,544 B** / 예산 33,554,432 B |

### 4. 검사가 없던 절반

`hygiene.test.ts`는 **CDN을 부르는지**는 검사하고 있었다. 검사하지 않던 것은 **스택이 시스템 폰트에
닿기 전에 벤더 서체에 닿는지**다. 그래서 `--font-ui`가 세 스킨 동안 `ui-sans-serif`로 시작한 채
...
## E-55 — **한자는 본명조 TC, 일본어는 본고딕 JP. 코드포인트가 아니라 이름 단위로 가른다** (사용자 요청, 2026-08-28) — BINDING

사용자 요청은 한 줄이었다.

> Noto Serif TC -> 한자, Noto Sans JP -> 일본어 기본폰트로 적용해줘요.

앞의 절반은 파일 두 개를 바꾸면 끝난다. 뒤의 절반은 **CSS가 할 수 없는 일이다.**

### 1. `unicode-range`로는 일본어를 못 가른다

E-53까지 CJK는 벤더 서체 **한 벌**(`Noto Serif KR Han`, 본명조 KR)이 한자와 가나를 모두
그렸고, 스택 분할은 전부 `unicode-range` — 즉 **코드포인트** 기준이었다. Hangul은 고운바탕,
latin은 고운바탕, 숫자는 Archivo. 이 방식이 통하지 않는 유일한 문자가 한자다.

**일본어 제목의 칸지와 한국어 제목의 한자는 같은 코드포인트다.** 「進撃の巨人」과 「三國志」를
가르는 것은 글자가 아니라 그 글자가 놓인 **이름**이다. 코드포인트로만 나누면 결과는 이렇게 된다.

```
進撃 の 巨人
└명조┘└고딕┘└명조┘     한 단어, 두 개의 스킨
```

그래서 판정을 **이름 단위**로 옮긴다. `lib/textLang.ts`가 가나를 가진 이름에 `lang="ja"`를
붙이고, base.css의 `[lang='ja']`가 그 요소의 스택을 `--font-ja`로 돌린다.

### 2. 판정

| | 서체 | 도달 경로 | 담는 것 |
|---|---|---|---|
| 한자 (기본) | **Noto Serif CJK TC** | `--font-heading`의 `unicode-range` | 한자 ∪ 가나 |
| 일본어 이름 | **Noto Sans CJK JP** | `[lang='ja']` → `--font-ja` | 한자 ∪ 가나 |

- **지역 컷이 KR에서 TC로 바뀐다.** 본명조는 같은 코드포인트를 지역마다 다르게 그린다. 한국어
  제목 속 한자는 전통 자형으로 쓰이므로 TC가 그 자형이다.
- **두 서체 모두 고운바탕이 스택 맨 앞이다.** 대칭을 맞추려는 게 아니다 — §3을 보라.
- **두 서체 모두 `unicode-range`가 Hangul에 닿지 않는다.** 같은 이유다.
- 가나를 TC에도 남겨 둔 것은 바닥이다. 태깅을 빠뜨린 이름이나 가나 없는 일본어 제목이
  두부(tofu) 대신 명조로라도 그려진다.

### 3. 실측이 설계를 바꿨다 — 태그는 대개 *한국어* 제목에 붙는다

`resources/shelf/index.db`의 이름 **18 721**개를 전수 분류했다.

| | 건수 | 비율 |
|---|---|---|
| 한자 포함 | 727 | 3.9 % |
| **가나 포함 (→ `ja` 판정)** | **128** | **0.68 %** |
| 한자만 (가나 없음) | 668 | 3.6 % |

128건의 생김새가 핵심이다. **순수 일본어 제목이 아니라, 일본어 조각이 박힌 한국어 제목이다.**

```
나와 그녀의 연애목록 (すかぢ) 1~19화
니노미아 히카루 - アイであそぶ 단편 모음집.zip
바다의 무녀 (완결, 미번) (文月晃, 海の御先)
```

그러니 `lang="ja"`가 붙는 문자열은 **대부분 한글**이다. 만약 본고딕이 스택을 이끌거나 Hangul을
`unicode-range`로 주장했다면, **괄호 하나 때문에 한국어 제목 전체가 고딕이 됐을 것이다.**
고운바탕이 맨 앞인 것과 두 서체가 Hangul을 건드리지 않는 것은 이 측정에서 나온 결론이다.

### 4. 명시된 한계 — 전(全)한자 일본어 제목

「攻殻機動隊」는 가나가 없으므로 `undefined`로 나오고 명조로 그려진다. 이건 이 휴리스틱이 고칠 수
있는 결함이 아니다. 그 문자열에는 「三國志」와 구별될 신호가 **없다.** 이름은 파일명에서 오고
메타데이터가 없다. 자형으로 추측하는 규칙을 쓰면 **한자를 가진 668개의 한국어 이름을 틀리게**
만들면서 소수를 얻는 거래가 된다. `textLang.test.ts`가 이 한계를 단언으로 박아 둔다 — 놀람이
아니라 기록된 판정이 되도록.
...
| | 이전 | 이후 |
|---|---|---|
| 벤더 CJK 서체 | 2 997 496 B | **5,579,784 B** (TC 3,306,136 + JP 2,273,648) |
| 벤더 서체 총계 | 3 936 288 B (3.94 MB) | **6,518,576 B (6.52 MB)** |
| linux/amd64 바이너리 (실측) | 30 351 544 B | **32 936 120 B** |
| `SIZE_BUDGET` 여유 | 3 202 888 B | **618 312 B** |

`unicode-range`가 이걸 감당 가능하게 만든다. 한자도 가나도 없는 화면은 두 파일 중 어느 것도
받지 않고, 한국어만 있는 라이브러리는 JP 쪽을 영원히 받지 않는다 — 이름의 0.68 %가 그 문을 연다.

**예산 여유가 3.2 MB에서 618 KB로 줄었다.** `SIZE_BUDGET`(33 554 432, E-19)은 그대로이고 게이트는
통과하지만, 다음 기능 하나가 이걸 깰 수 있다. 그리고 Makefile 주석이 기본 빌드를
`25 833 656`이라고 적어 둔 것은 **이 변경 전에 이미 4.5 MB 밀려 있던 숫자다** — 실측 HEAD가
30 351 544 B다. 예산 자체는 `release_budget_test.go`가 문서 넷 사이에서 지키지만, *측정치* 를
지키는 것은 아무것도 없다.

### 6. 이 변경이 CSS 유닛 티어를 통째로 통과한 채 틀렸다

`[lang='ja']` 규칙을 처음에 `@layer components`에 넣었다. `.card-title`과 `.btn`을 이기려는
의도였고, 근거도 적어 두었다. **브라우저에서는 졌다.**

`FallbackCover`·`ContinueCard`·`NextVolumeCard`·`ViewerTopBar`는 태그를 다는 바로 그 요소에
`font-heading` **유틸리티**를 함께 붙인다. Tailwind는 유틸리티를 `@tailwind components` **뒤에**
찍고, 명시도가 같으면 뒤가 이긴다. 그래서 네 표면이 `lang="ja"`를 달고도 계속 명조로 그려졌다.

**아무 검사도 죽지 않았다.**

| 티어 | 왜 못 보나 |
|---|---|
| `tokens.test.ts` | 시트를 **텍스트로** 읽는다. 규칙이 있다는 것만 안다 |
| `nameLang.test.tsx` | vitest가 `css: false` — 스타일시트를 **적용하지 않는다** |
| `hygiene.test.ts` | 스택의 선두가 벤더 서체인지만 본다 |

세 티어 전부 "규칙이 존재한다"를 검사하고 있었고, 문제는 **어느 규칙이 이기느냐**였다.
[[checks-that-watch-the-wrong-thing]]

실서버(:9909)에 vite를 프록시로 물려 `getComputedStyle`을 읽고 나서야 드러났다.
규칙을 **모든 레이어 밖, 파일 맨 끝**으로 옮겼다 — `@tailwind utilities` 전개보다 뒤이므로
`font-heading`에 닿는다.

새 검사는 소스가 아니라 **컴파일된 시트**를 본다. `styles/cascade.test.ts`가 실제
PostCSS/Tailwind 파이프라인을 돌려, 출력에서 `[lang='ja']`가 모든 `font-*` 유틸리티보다 **뒤에**
오는지, 그리고 그 뒤에 `font-family`를 설정하는 클래스 규칙이 하나도 없는지 단언한다. 규칙을
`@layer components`로 되돌리는 뮤테이션에서 두 케이스 모두 죽는다 — `tokens.test.ts` 80건은
초록인 채로.

### 7. 부수 발견 — 검사 셋이 각각 엉뚱한 것을 보고 있었다

**(a) `subset.py`는 이 트리에서 한 번도 완주한 적이 없다.** E-53이 "실행 가능한 레시피"로 쓴
스크립트가 `@fontsource/archivo`를 참조하는데, 그 의존성은 **커밋 `97e9782`(서고 리페인트)에서
package.json에서 제거됐다.** 스크립트는 항상 exit 2로 죽었고 `--check`도 마찬가지였다.
의존성을 스크립트가 이미 못박아 둔 버전으로 복구했다 — package.json +1줄, 락파일 +8줄, 다른
패키지 변동 없음. 이제 `--check`가 처음으로 끝까지 통과한다.

**(b) 서브셋은 요청한 글자가 실제로 들어갔는지 확인한 적이 없다.** `pyftsubset`은 원본에 없는
글자를 조용히 버린다. 그래서 원본 서체를 KR → TC로, Serif → Sans로 바꾸는 이번 변경은 원리적으로
**아무 소리 없이 구멍을 낼 수 있었다.** `missing_glyphs()`를 넣어 결과 woff2의 cmap을 다시 읽고
대조한다 — 요청이 아니라 산출물을 본다. 넣자마자 값을 했다: `぀ ゗ ゘`(U+3040, U+3097, U+3098)는
**유니코드 미할당 코드포인트**로 어떤 폰트에도 없다. KR 서브셋에도 없었지만 아무도 몰랐다.

**(c) 새로 쓴 DOM 테스트가 뮤테이션에서 살아남았다.** `SeriesCard`의 `lang`을 떼도 통과했다.
헬퍼가 `getAllByText(name)[0]`을 집었는데 **카드는 이름을 두 번 그린다** — 밑의 `FallbackCover`와
그 위의 제목. `[0]`은 FallbackCover의 태그를 읽고 있었고, 결국 그 단언은 네 개의 다른 이름으로
`FallbackCover` 하나를 네 번 검사한 셈이었다. 이름을 그리는 **모든** 요소를 모아 검사하도록
고치자 네 표면 전부 뮤테이션에서 죽는다.

**(d) 인구조사 정규식이 한글을 통째로 삼켰다 — 동형자 하나 때문에.** 같은 조사를 두 번 돌렸는데
한 번은 1 112개 시리즈 중 **1 100개**가 한자를 가졌다고 나왔다. `豈`가 U+F900(호환 한자)이 아니라
**U+8C48(통합 한자)** 이었고, 그래서 `豈-﫿`가 U+8C48–U+FBFF, 즉 **한글 음절 전체(U+AC00–U+D7A3)를
포함하는 범위**가 됐다. 두 글자는 화면에서 구별되지 않고 diff에도 아무것도 안 보인다.
§3의 숫자는 코드포인트 이스케이프로 다시 재서 확인했다(727 / 128 / 668, 고유 한자 111 · 가나 69,
전부 닫힌 집합 안). **출하되는 `textLang.ts`의 정규식도 리터럴에서 이스케이프로 바꿨고**,
`textLang.test.ts`가 BMP 전역에서 규칙을 **숫자로 쓴 명세**와 대조한다 — 같은 문자 리터럴로 쓴
테스트는 규칙과 구조적으로 합의해 버려서 이 결함을 볼 수 없다.

### 8. 실증 — 실서버, 실제 이름

사용자 서버(:9909)를 재시작하지 않고, 새 프런트를 vite로 띄워 `/api`만 프록시해 읽기 전용으로 확인했다.

| 검사 | 결과 |
|---|---|
| `나와 그녀의 연애목록 (すかぢ) 1~19화` 의 `h2` | `lang="ja"`, computed `--font-ja` 스택 |
| 같은 이름의 `FallbackCover` div (`font-heading` 보유) | 수정 후 `--font-ja` (수정 전 `--font-heading`) |
| `군계 1~25` (대조군) | `lang` 없음, `--font-heading` 스택 |
| 실제 내려받은 woff2 | `noto-serif-tc-400/700`, `noto-sans-jp-700` — 화면이 요구할 때만 |
| 한글 렌더 | 두 경우 모두 고운바탕 — 본고딕이 한글을 가져가지 않는다 |
| 바이너리 | 32 936 120 B, 예산 33 554 432 B 이내 (HEAD 실측 30 351 544 B) |

### 9. 재시작 뒤에 드러난 것 — 낙관 서브셋이 제목의 한자를 가로채고 있었다

사용자가 서버를 재시작한 뒤 라이브에서 다시 재다가, **`完`이 TC가 아니라 두 글자짜리 낙관
서브셋으로 그려지고 있는 것**을 찾았다.

`seal-han-bold`(藏)와 `seal-han-done`(完讀)은 E-46에서 **`Gowun Batang`이라는 이름으로**
선언되고 `unicode-range`로 울타리를 쳐, *"경쟁하지 않고 스택에 합류한다"* 는 설계였다. 그건
고운바탕 뒤에 그 글자를 그릴 수 있는 것이 아무것도 없을 때만 안전한 배치다. **E-55가 그 전제를
깼다** — `--font-heading`의 선두에 있는 패밀리는 자기가 주장하는 모든 문자에 대해 우선권을
가지므로, 손으로 고른 세 글자가 *제목*의 한자를 벤더 본명조보다 먼저 답하고 있었다.

가정이 아니다. **`完`은 이 라이브러리의 완결 표기**이고, 한자를 가진 이름 727개 중 **204개**에
들어 있다. 그 204개는 한자를 **Noto Serif CJK KR**에서 잘라낸 2글자 서브셋으로 그리면서 같은
화면의 다른 이데오그래프는 TC로 그리고 있었고, 낙관에는 400 굵기가 없으므로 **같은 글자가 본문
굵기에 따라 KR과 TC로 갈렸다.** KR과 TC의 `完`은 CID도 아웃라인도 실제로 다르다(`三`·`國`처럼
같은 글자도 있지만 `完`·`志`·`直`·`骨`·`花`는 다르다).

**고침은 전용 패밀리 `Shelf Seal`이다.** 낙관 컷은 `--font-seal`을 통해서만 닿고,
`Wordmark.tsx`와 `DoneSeal.tsx`가 그 유일한 호출자다. **자형은 KR 컷 그대로 둔다** — 이건 제품이
*그리는* 마크이고 E-46이 프로토타입이 찍는 바로 그 글자로 못박았다. 로고가 본문과 같은 지역 컷을
쓸 의무는 없다.

검사는 두 패밀리 이름이 아니라 **규칙**으로 썼다: *개별 코드포인트로 제한된 서체는 산문을 세우는
스택(`--font-heading`·`--font-body`·`--font-ui`·`--font-ja`)이 이름을 부르는 패밀리 아래 선언될
수 없다.* 옛 배치로 되돌리는 뮤테이션에서 죽는다.

**이것도 §6과 같은 이야기다.** 유닛 티어는 규칙이 존재하는지만 보고, 어느 서체가 실제로 그 글자를
그렸는지는 `document.fonts`를 읽어야 알 수 있다. 라이브 서버에서 `Noto Serif TC/700`이 로드
목록에 없다는 한 줄이 단서였다.

### 10. 되돌리려면

`--font-heading`의 `'Noto Serif TC'`를 `'Noto Serif KR Han'`으로 되돌리고, base.css 끝의
`[lang='ja']` 규칙을 지우고, `Shelf Seal`을 다시 `Gowun Batang`으로 합치면 E-53 상태다.
그러면 `tokens.test.ts`의 E-55 케이스 둘, `cascade.test.ts` 2건, `nameLang.test.tsx` 8건이
죽는다 — 의도적으로, 그 자리에서.
