...
전체 페이지를 가상화하고(주석 `:10-14`가 *"프로토타입은 60에서 자른다"*고 명시), `:94-97`이 `current`가
바뀔 때마다 `align:'center'`로 리센터한다. **반영할 것이 없다.** 남은 진짜 델타는 **애니메이션의 구분**
하나다.
...
끄고, 두 바와 권 끝 카드의 `::after`로 크롬에만 재도포한다. 스트립은 하단 바의 자식이므로 셋이 넷을
덮는다 — **스트립 자체에 걸면 가로 스크롤과 함께 흘러간다.** 권 끝 카드에는 `position: relative`가
...
`ui-spec` §0의 다섯 항목 중 **둘**이 E-32의 사후 정정을 달게 됐다.
