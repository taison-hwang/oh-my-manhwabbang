...
| **D-7** | **Non-media series exist** — 5 top-level dirs hold only `.txt`/~~`.hv3`~~. | Indexed and listed greyed out, never silently dropped (arch OQ-6); `?status=ok` hides them. The status is `empty` only when there is nothing in there — a child holding a format this build recognises and cannot open reports **`unsupported`** and names it (**D-72**). **Narrowed by E-51:** the `.hv3` half is gone. `.hv3` is a container with a reader now, so a directory holding one has a *book* in it — `ok` if it opens, `error` if it does not, exactly as a 0-byte `.zip` is treated. The `.txt` half is unchanged and is what still makes this row bite. |
...
| I-5 | **AC-001 / NFR-PRF-006** | read all 1 540 pages of `[만화] 배틀로얄 1~15 [완결].zip`: peak RSS growth < 64 MiB, and `find $TMPDIR $PWD -newer <marker>` outside `cache_dir` is empty. *(Since **D-73** those 1 540 pages are the archive's fifteen 권 rather than one book of 1 540; the run reads the whole archive, volume by volume, and measures across the lot.)* |
...
| 9 | `배틀로얄 1~15 [완결].zip` | one 1.34 GB ZIP, **1 540 pages in 15 per-volume directories** | **AC-008**; NFR-PRF-006; deflate streaming at scale. Since **D-73** it is also the real-collection sample of a container of chapter directories — 15 `kind:"nesteddir"` 권, page total unchanged. The one remaining name holding a `[`, so it is the only reason `scripts/e2e-config.sh`'s glob escaping is still load-bearing. **Step 6.9's "the strip must not mount 1 540 cells" lost its subject with the split** — no curated book now exceeds the 120-cell window; `이누야샤 01~56권 완결/이누야샤(1~56권)(완).zip` (7 480 pages, 1.40 GB, measured flat) or 군계's 207-page `03권` is the replacement if the subset is re-based |
...
| 15 | `펌프킨 시저스 1~13권` | folder of ZIP volumes, one of which — `펌프킨 시저스 04.zip` — holds 39.5 MB in a single `.hv3` | **E-51** — the `.hv3` is a `nestedhv3` volume of **104 pages**, verified against the CRC-32 the container itself records for every one. Three rulings have been about this file: `비어 있음` under D-07 (false), `unsupported` naming HV3 under **D-72** (true while the format was believed encrypted), and readable under E-51. D-72's rule is still exercised by `사모님은 학생회장.zip`'s `.7z`. |
...
5. **curl assertions** (no browser yet): `GET /api/series` returns `total == 10`; kinds are `folder×6, zip×3, pdf×1`; `[만화] 디엔엔젤…` has exactly one book with `status:"error"`; `[만화] 엔젤하트…` is series `status:"error"` with a non-empty `error` (its single book is `status:"empty"` — D-10 as narrowed by **E-14**); `[만화] 군계 1~25` has ≥ 27 books including two with `status:"error"` and both `01권` variants; **no page name in a 500-name sample contains `�`**; the archive's **900th page** returns `200 image/jpeg` in < 200 ms — since **D-73** that page is `/api/books/{battle_royale 09권}/pages/…` rather than `{battle_royale}/pages/900`, the same offset in the same 1.34 GB file addressed by the 권 that holds it.
