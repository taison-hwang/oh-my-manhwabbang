...
| **D-7** | **Non-media series exist** — 5 top-level dirs hold only `.txt`/`.hv3`. | Indexed and listed greyed out, never silently dropped (arch OQ-6); `?status=ok` hides them. The status is `empty` only when there is nothing in there — a child holding a format this build recognises and cannot open reports **`unsupported`** and names it (**D-72**). The `.hv3` that made this row is now a single entry inside `펌프킨 시저스 04.zip`, and it is encrypted. |
