...
*A-9 supersedes A-7's "frozen verbatim" for §7.2's enum only, A-10 for §7.8's `server` block only, and A-11
for §7.2's enum again plus §7.4's endpoint table, §7.8's `server` block and §3.2's `server:` block — exactly as
A-7 requires in all three cases: the table is updated first.*
...
document (**A-1 … A-10**) and the enum resolutions C-1 … C-4. Read it there; it is not duplicated here so
...
| 10 | `[만화] 엔젤하트 전32권 완결.zip` | 1.44 GB ZIP containing 33 sub-ZIPs, 0 images | book `status:"empty"`, series **`status:"error"`** with that reason (D-10 as narrowed by **E-14**); proves nested archives are out of scope without crashing, and that a series with nothing readable in it is visibly broken rather than greyed out |
