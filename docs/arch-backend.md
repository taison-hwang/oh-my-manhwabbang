...
| Non-media clutter | `.hv3` 26, `.txt` 19, `.rar` 8, `.part` 2, `.info` 1 |
...
| **OQ-6** | **Non-media series.** 5 top-level directories contain only `.txt`/`.hv3` (text novels, a proprietary format) — 26 `.hv3` and 19 `.txt` files. | **List them with `status:"empty"`** and let the frontend grey them out. Hiding directories the user can see in their file manager is more confusing than showing them. Add `?status=ok` for a clean view. |
